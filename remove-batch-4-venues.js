...
// Batch 4 venues to remove (not actual golf simulators)
const venuesToRemove = [
  'Caddyshackers',
  'Glory Holes Golf',
  'Horton\'s Emporium Escape Rooms',
  'Funstation Group',
  'Active Golf Projects'
];

async function removeBatch4Venues() {
  console.log('🗑️  Removing Batch 4 Non-Simulator Venues\n');
  ...
  try {
    ...
    const backupFile = `/Users/edwelton/Documents/Welto Digital/find-a-golf-range/batch-4-removed-venues-backup-${timestamp}.json`;

    const backup = {
      timestamp: new Date().toISOString(),
      reason: 'Batch 4 removal - Additional non-simulator venues (escape rooms, entertainment, etc.)',
    ...
    // Save results
    const resultsFile = `/Users/edwelton/Documents/Welto Digital/find-a-golf-range/batch-4-removal-results-${timestamp}.json`;
    ...
    console.log(`Batch 4 (Current): ${successfulRemovals} venues removed`);
    console.log(`Total removed: ${15 + successfulRemovals} venues`);
    ...
    console.log('Removed escape rooms, entertainment venues, and non-simulator facilities');
...
removeBatch4Venues().catch(console.error);