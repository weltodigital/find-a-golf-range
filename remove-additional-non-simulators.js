...
const venuesToRemove = [
  ...
  'Bogeys Golf',
  'Glow in One',
  'Home Cinema - Living Home Tech',
  'Wedogolf Ltd',
  'Golf Fang Birmingham'
];

async function removeAdditionalNonSimulators() {
  ...
  try {
    ...
    const backupFile = `/Users/edwelton/Documents/Welto Digital/find-a-golf-range/batch-3-removed-venues-backup-${timestamp}.json`;

    const backup = {
      timestamp: new Date().toISOString(),
      reason: 'Batch 3 removal - Additional non-simulator venues (mini golf, entertainment, etc.)',
    ...
    // Save results
    const resultsFile = `/Users/edwelton/Documents/Welto Digital/find-a-golf-range/batch-3-removal-results-${timestamp}.json`;
    ...
    console.log(`Batch 3 (Current): ${successfulRemovals} venues removed`);
    console.log(`Total removed: ${6 + successfulRemovals} venues`);
    ...
    console.log('Removed mini golf, entertainment, and non-simulator facilities');
...
removeAdditionalNonSimulators().catch(console.error);