...
// Batch 5 venues to remove (not actual golf simulators)
const venuesToRemove = [
  'The Hole In Wand',
  'KK\'s Krazy Golf & Sports Bar',
  'Kiddy Kids',
  'Hukd Crazy',
...
async function removeBatch5Venues() {
  console.log('🗑️  Removing Batch 5 Non-Simulator Venues\n');
  ...
  try {
    ...
    const backupFile = `/Users/edwelton/Documents/Welto Digital/find-a-golf-range/batch-5-removed-venues-backup-${timestamp}.json`;

    const backup = {
      timestamp: new Date().toISOString(),
      reason: 'Batch 5 removal - Non-simulator venues (mini golf, children\'s farms, entertainment venues)',
    ...
    // Save results
    const resultsFile = `/Users/edwelton/Documents/Welto Digital/find-a-golf-range/batch-5-removal-results-${timestamp}.json`;
    ...
    console.log(`Batch 5 (Current): ${successfulRemovals} venues removed`);
    console.log(`Total removed: ${21 + successfulRemovals} venues`);
    ...
    console.log('Removed mini golf, children\'s entertainment, and non-simulator facilities');
    console.log('Map accuracy improved with only genuine simulator locations');
...
removeBatch5Venues().catch(console.error);